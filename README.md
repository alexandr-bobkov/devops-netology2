...
В ходе выполнения задания локальный репозиторий был успешно связан с двумя удаленными серверами:
1. `origin` — репозиторий на GitHub.
2. `gitlab` — репозиторий на GitLab.

При работе с GitLab была решена проблема авторизации по протоколу HTTPS через создание личного токена доступа (**Personal Access Token**) с областями видимости `read_repository` и `write_repository`.
...
На коммите `5e9d68e` были созданы два типа тегов и отправлены на удаленные сервера:
1. **Легковесный тег `v0.0`** — является простой ссылкой на коммит. При вызове `git show v0.0` отображает только данные коммита.
2. **Аннотированный тег `v0.1`** — сохранен как полноценный объект. При вызове `git show v0.1` в терминале явно отображаются уникальные метаданные автора тега (`Tagger: Alexandr Bobkov`), дата его создания и сообщение: *"Release version 0.1 с домашним заданием"*.

В веб-интерфейсе GitHub аннотированный тег `v0.1` отличается тем, что выводит добавленное к нему текстовое сообщение релиза на странице Tags.
> ** Создание и отображение тегов GITHUB и GITLAB:** 📸
> ![Скриншот](./img/03.png)
> ![Скриншот](./img/2.png)
> ![Скриншот](./img/3.png)
> ![Скриншот](./img/4.png)
...
В рамках моделирования ситуации с исправлением ошибки в продакшене были выполнены следующие шаги:
1. Осуществлен переход на коммит назад во времени — к коммиту `Prepare to delete and move` (`git checkout 5e9d68e`).
2. На базе этого коммита создана новая изолированная ветка `fix` (`git switch -c fix`).
3. Ветка была опубликована на GitHub (`git push -u origin fix`).
4. В файл `README.md` внутри ветки `fix` внесены изменения, создана новая запись и зафиксирована коммитом.

Лог коммитов репозитория (`git log --oneline`) в ветке `fix`:
> ** Процесс создания новой "ответвленной" ветки от нужного коммита:** 📸
> ![Скриншот](./img/5.png)
> ** Созданная ветка `fix` в GITHUB:** 📸
> ![Скриншот](./img/8.png)

В результате на GitHub сформировался наглядный граф ветвления, где ветка `fix` успешно отделилась от истории `main`.
...
> ** Делаю изменения в файле `README.md`** 📸
> ![Скриншот](./img/10.png)
